...
export const changeSessionStatus = async (sessionId, status) => {
  try {
    ...
    const statusToChange = status.status;

    const validStatus = ['PENDING', 'PAYMENT_PENDING', 'ONGOING', 'COMPLETED', 'CANCELLED', 'TRIAL', 'APPROVED'];
    if (!validStatus.includes(statusToChange)) {
      return { status: false, message: 'Invalid status' };
    }

    const updatedSession = await Session.findByIdAndUpdate(sessionId, { status : statusToChange }, { new: true });
    ...
    return { status: false, message: 'Error updating session status' };
  }
};
